...
var getQToken = function () {
  ...
  var url = credentials.churchtools.host + '/api/csrftoken';
    logger.debug('74AB9450-438F-4476-93E6-3EC251539EB1', 'Calling CT API api/csrftoken url: ' + url);
        logger.debug('74AB9450-438F-4476-93E6-3EC251539EB1', 'Calling CT API api/csrftoken cookies: ' + cookies);
  logger.debug('74AB9450-438F-4476-93E6-3EC251539EB1', 'Calling CT API api/csrftoken ...');
  rest.get(url, {
    headers: {
      Cookie: cookies
    }
    ...
    if (!res) {
      ...
      logger.debug('E6C427A6-D45B-4610-A0EB-36317B138D13', 'Response from CT call api/csrftoken : no response!', {result: result});
      var thenable = Q(true)
    ...
    logger.debug('4FDE9C12-A090-49ED-993C-D723D5EB2CDE', 'Response from CT call api/csrftoken : '+res.statusCode);
    ...
    if (result.data) {
      ...
      deferred.reject({message: 'Call to CT, api/csrftoken, failed: '+(result ? JSON.stringify(result) : 'no result')});
    }
  }).on('timeout', function () {
    logger.debug('54CBEDCB-204B-4AA7-8D13-610C0E31DBA3', 'Timeout at CT call api/csrftoken !');
    var thenable = Q(true)
      .delay(1000) // wait one second, then try again
      .then(function () {
        // FIXME: check here, if we should abort instead of trying again
...
var postQ = function (q, data, contentType, multipart, retry) {
  ...
  logger.debug('74AB9450-438F-4476-93E6-3EC251539EB1', (retry?'Retry: ':'')+'Calling CT API '+q+' CSRF Token: '+csrfToken);
