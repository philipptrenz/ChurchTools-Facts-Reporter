...
function setLoginData(host, user, password, savePassword) {
  ...
  .then(() => {
      if (savePassword) return keytar.setPassword(appName, user, password);
      else return keytar.deletePassword(appName, user);
  });
}
...
function getLoginData() {
  ...
  .then(() => {
    return keytar.getPassword(appName, churchtools.user)
  })
  .then((pwd) => {
...
function deletePassword() {
    return keytar.deletePassword(appName, churchtools.user);
}
